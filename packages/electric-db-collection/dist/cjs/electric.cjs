...
function isSubsetEndMessage(message) {
  return client.isControlMessage(message) && message.headers.control === `subset-end`;
}
  ...
  const loadSubset = async (opts) => {
    if (isBufferingInitialSync()) {
      ...
      if (cursor) {
        ...
        const whereCurrentParams = sqlCompiler.compileSQL(whereCurrentOpts);
        ...
        const whereFromParams = sqlCompiler.compileSQL(whereFromOpts);
...
function createElectricSync(shapeOptions, options) {
  ...
  const addTagsToRow = (tags, rowId, rowTagSet) => {
    for (const tag of tags) {
      ...
      if (tagLength === void 0) {
        tagLength = tagIndex.getTagLength(parsedTag);
        initializeTagIndex(tagLength);
      }
      const currentTagLength = tagIndex.getTagLength(parsedTag);
      ...
      rowTagSet.add(tag);
      tagIndex.addTagToIndex(parsedTag, rowId, tagIndex$1, tagLength);
    }
  };
  const removeTagsFromRow = (removedTags, rowId, rowTagSet) => {
    ...
    for (const tag of removedTags) {
      ...
      tagIndex.removeTagFromIndex(parsedTag, rowId, tagIndex$1, tagLength);
      tagCache.delete(tag);
    }
  };
  ...
  const removeMatchingTagsFromRow = (rowId, pattern) => {
    ...
    for (const tag of rowTagSet) {
      const parsedTag = parseTag(tag);
      if (tagIndex.tagMatchesPattern(parsedTag, pattern)) {
        rowTagSet.delete(tag);
        tagIndex.removeTagFromIndex(parsedTag, rowId, tagIndex$1, tagLength);
      }
    }
  ...
  const processMoveOutEvent = (patterns, begin, write, transactionStarted) => {
    ...
    for (const pattern of patterns) {
      const affectedRowIds = tagIndex.findRowsMatchingPattern(pattern, tagIndex$1);
...
You can provide an 'onError' handler on the shapeOptions to handle this error, and this message will not be logged.`,
      ...
      unsubscribeStream = stream.subscribe((messages) => {
        ...
        for (const message of messages) {
          if (client.isChangeMessage(message) || tagIndex.isMoveOutMessage(message)) {
          ...
          } else if (tagIndex.isMoveOutMessage(message)) {
            if (isBufferingInitialSync() && !transactionStarted) {
              bufferedMessages.push(message);
            } else {
        ...
        if (commitPoint !== null) {
          if (isBufferingInitialSync() && commitPoint === `up-to-date` && !transactionStarted) {
            ...
            for (const bufferedMsg of bufferedMessages) {
              ...
              } else if (tagIndex.isMoveOutMessage(bufferedMsg)) {
                processMoveOutEvent(
