...
function createLoadSubsetDedupe({
  ...
  const loadSubset = async (opts) => {
    if (isBufferingInitialSync()) {
      const snapshotParams = compileSQL(opts, { encode });
      try {
      ...
      if (cursor) {
        ...
        const whereCurrentParams = compileSQL(whereCurrentOpts, { encode });
        ...
        const whereFromParams = compileSQL(whereFromOpts, { encode });
